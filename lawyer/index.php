...
$APPLICATION->SetPageProperty("description", 'Договоры');
...
<main class="main">
	<div class="container">
			...
			<div class="filter">
				...
				<div class="filter-body">
					...
					<form class="d-flex flex-column gap-2" method="post">
						<select id="name-select" class="form-select" name="client">
							...
							if ($iblock) {
								...
								if ($property) {
									...
									foreach ($propertyEnum as $enum) {
										?>
										<option value="<?= $enum['ID'] ?>" <?= $enum['ID'] == $_REQUEST["type"] ? "selected" : "" ?>><?= $enum['VALUE'] ?></option>
										<?
									}
								}
							}
						...
						<input class="form-control" type="date" name="date" placeholder="Дата создания договора" value="<?= $_REQUEST["date"] ?>">
						<select class="form-select" name="status_contract">
							<option value="">Статус договора</option>
							<?
							$property = \CIBlockProperty::GetList([], ['CODE' => 'STATUS', 'IBLOCK_ID' => $iblockId])->Fetch();
							if ($property) {
								$propertyId = $property['ID'];
								...
								foreach ($propertyEnum as $enum) {
									?>
									<option value="<?= $enum['ID'] ?>" <?= $enum['ID'] == $_REQUEST["status_contract"] ? "selected" : "" ?>><?= $enum['VALUE'] ?></option>
									<?
								}
							}
							?>
						</select>
						<select class="form-select" name="status_check_contract">
							<option value="">Статус проверки договора</option>
							<?
							$property = \CIBlockProperty::GetList([], ['CODE' => 'STATUS_CHECK', 'IBLOCK_ID' => $iblockId])->Fetch();
							if ($property) {
								$propertyId = $property['ID'];
								...
								foreach ($propertyEnum as $enum) {
									?>
									<option value="<?= $enum['ID'] ?>" <?= $enum['ID'] == $_REQUEST["status_check_contract"] ? "selected" : "" ?>><?= $enum['VALUE'] ?></option>
									<?
								}
							}
							?>
						...
						<select class="form-select" name="contract_signer" id="contract_signer">
							<option value="">Заключивший договор</option>
							<?
							$groupCode = "director|chief_lawyers|senior_lawyers|junior_lawyers";
							$rsGroup = CGroup::GetList($by = "c_sort", $order = "asc", array("STRING_ID" => $groupCode));
							$groupIds = array();
						...
						<input class="form-control" type="text" name="sum" placeholder="Сумма платежа" value="<?= $_REQUEST["sum"] ?>">
		...
		<div class="d-flex justify-content-between align-items-end my-3">
			...
			<? if(checkAccess(2)): ?>
			<a class="btn btn-primary create-client-btn" href="/lawyer/create/">
				<img class="mx-2" width="24" height="24" src="/local/templates/crm-my-prava/images/user/create-client.png" alt="">
				Создать клиента
			</a>
			<? endif; ?>
		</div>
		...
		{
			$arFilter["DATE_ACTIVE_FROM"] = FormatDate("d.m.Y", strtotime($_REQUEST["date"]));
		}
		if(!empty($_REQUEST["status_contract"]))
		{
			$arFilter["PROPERTY_STATUS"] = $_REQUEST["status_contract"];
		}
		if(!empty($_REQUEST["status_check_contract"]))
		{
			$arFilter["PROPERTY_STATUS_CHECK"] = $_REQUEST["status_check_contract"];
		}
		if(!empty($_REQUEST["responsible"]))
		{
			$arFilter["PROPERTY_LAWYER"] = $_REQUEST["responsible"];
		}
		if(!empty($_REQUEST["contract_signer"]))
		{
			$arFilter["PROPERTY_CREATOR"] = $_REQUEST["contract_signer"];
		}
		if(!empty($_REQUEST["sum"]))
		{
			$arFilter["PROPERTY_CONTRACT_AMOUNT"] = $_REQUEST["sum"];
		}
